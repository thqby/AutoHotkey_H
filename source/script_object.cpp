...
ResultType Object::CallHiddenMethod(int aFlags, LPTSTR aName, ResultToken &aResultToken, ExprTokenType &aThisToken, ExprTokenType *aParam[], int aParamCount)
{
	...
	{
		if (aName && !ParamIndexIsOmitted(0) && !_tcsicmp(aName, _T("Get")))
		{
			result = CallHiddenMethod(IT_GET | EIF_UNSET_RETURN, nullptr, aResultToken, aThisToken, aParam, 1);
			if (aResultToken.symbol == SYM_MISSING && !ParamIndexIsOmitted(1))
				aResultToken.CopyValueFrom(*aParam[1]);
		}
	}
	else if (!aName && aParamCount == (IS_INVOKE_SET ? 2 : 1))
	{
		...
		if (result == INVOKE_NOT_HANDLED && IS_INVOKE_GET)
		{
			if (aFlags & EIF_UNSET_RETURN)
				aResultToken.symbol = SYM_MISSING, result = OK;
	...
	else if (!aParamCount && ((aFlags & (IT_BITMASK | IF_IGNORE_DEFAULT | EIF_UNSET_RETURN)) == IT_GET) && g_DefaultObjectValue)
		result = aResultToken.ReturnPtr(g_DefaultObjectValue);
