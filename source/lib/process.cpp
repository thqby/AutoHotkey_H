...
static FResult ProcessWait(StrArg aProcess, optl<double> aTimeout, UINT &aRetVal, bool aWaitClose)
{
	...
	DWORD start_time = GetTickCount(), aThreadID = CURRENT_THREADID;
		...
		if (wait_indefinitely || (remainder = sleep_duration - (GetTickCount() - start_time)) > SLEEP_INTERVAL_HALF)
		{
			if (aWaitClose && (proc || (proc = OpenProcess(SYNCHRONIZE, FALSE, pid))))
			{
				...
				case WAIT_OBJECT_0 + 1:
					...
					else MsgSleepWithListLines(-1, waiting_line, start_time);
					continue;
				case WAIT_TIMEOUT:
					CloseHandle(proc);
					aRetVal = pid;
			...
			if ((char)g->IsPaused == -1)
				...
				Sleep(SLEEP_INTERVAL);
			else MsgSleepWithListLines(100, waiting_line, start_time);  // For performance reasons, don't check as often as the WinWait family does.
		}
		else // Done waiting.
		{
			if (proc)
