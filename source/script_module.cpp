...
thread_local Object *ScriptModule::sPrototype;
...
void ScriptModule::Clear()
{
	...
	for (auto &line = mFirstLine; line; line = line->mNextLine)
		line->Free();
	free(mVars.mItem);
	free(mFiles);
	mVars = {};
...
void ScriptModule::Free(bool aIsConstant)
{
	auto prev = g->ExcptMode;
	g->ExcptMode |= EXCPTMODE_DEBUGGER;
	for (auto i = 0; i < mVars.mCount; i++)
	{
		Var &aVar = *mVars.mItem[i];
		if ((aVar.mType == VAR_CONSTANT) != aIsConstant)
			continue;
