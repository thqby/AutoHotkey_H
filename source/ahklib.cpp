...
#define AHKAPI(_rettype_) extern "C" _rettype_ __stdcall


thread_local IDispatch *lib_ProblemCallback = nullptr;
...
public:
	EnumVARIANT() : mRefCount(1), mIndex(0) {}
...
public:
	...
	STDMETHODIMP Next(ULONG celt, /*out*/ VARIANT *rgVar, /*out*/ ULONG *pCeltFetched)
	{
		...
		for (i = 0; i < celt && mIndex < (ULONG)g_script->mFuncs.mCount; ++i, ++mIndex)
		{
			...
			auto hr = DescribeFunc::Create(g_script->mFuncs.mItem[mIndex], &rgVar[i].pdispVal);
			if (FAILED(hr))
			{
				if (pCeltFetched)
					*pCeltFetched = i;
...
public:
	...
	STDMETHODIMP Next(ULONG celt, /*out*/ VARIANT *rgVar, /*out*/ ULONG *pCeltFetched)
	{
		...
		{
			rgVar[i].vt = VT_DISPATCH;
			auto hr = DescribeParam::Create(mFunc->mParam + mIndex, mIndex == mFunc->mParamCount, &rgVar[i].pdispVal);
			if (FAILED(hr))
			{
				if (pCeltFetched)
					*pCeltFetched = i;
...
{
public:
	AutoHotkeyLib(ITypeInfo *typeInfo) : DispObject<IAutoHotkeyLib>(typeInfo) {}
	...
	STDMETHODIMP LoadFile(BSTR aScriptPath)
	{
		if (Line::sSourceFileCount)
			return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
		EarlyAppInit();
		if (!g_script->Init(aScriptPath, nullptr)
			|| g_script->LoadFromFile(aScriptPath) == LOADING_FAILED
			|| !InitForExecution())
		{
			g_script->TerminateApp(EXIT_CRITICAL, 0);
			return E_FAIL;
		}
		return S_OK;
	}
	...
	STDMETHODIMP NewThread(BSTR Script, VARIANT CmdLine, VARIANT Title, IDispatch **Value) {
		LPTSTR p[2];
		VARIANT *param[] = { &CmdLine,&Title };
		for (int i = 0; i < 2; i++)
		{
			auto &t = param[i];
			if (t->vt == VT_BSTR)
				p[i] = t->bstrVal;
			else if (t->vt == VT_ERROR && t->scode == DISP_E_PARAMNOTFOUND)
				p[i] = nullptr;
			else
				return DISP_E_TYPEMISMATCH;
		}
		LPSTREAM stream = nullptr;
		::NewThread(Script, p[0], p[1], FALSE, [](void *param) {
			if (!g_script->mIsReadyToExecute)
				return;
			IDispatch *obj;
			if (SUCCEEDED(CreateDispatchInstance<AutoHotkeyLib>(&obj)))
			{
				FAILED(CoMarshalInterThreadInterfaceInStream(IID_IDispatch, obj, (LPSTREAM *)param));
				obj->Release();
			}
		}, &stream);
		if (stream)
			if (SUCCEEDED(CoGetInterfaceAndReleaseStream(stream, IID_IDispatch, (void**)Value)))
				return S_OK;
			else stream->Release();
		return E_FAIL;
	}
	...
	STDMETHODIMP FindLabel(BSTR LabelName, IDispatch **ppLabel)
	{
		if (!g_script)
			return E_FAIL;
		if (auto label = g_script->FindLabel(LabelName))
			return DescribeLabel::Create(label, ppLabel);
		return DISP_E_UNKNOWNNAME;
	}


	STDMETHODIMP FindFunc(BSTR FuncName, IDispatch **ppFunc)
	{
		if (!g_script)
			return E_FAIL;
		if (auto func = g_script->FindGlobalFunc(FuncName))
			return DescribeFunc::Create(func, ppFunc);
		return DISP_E_UNKNOWNNAME;
	}

	STDMETHODIMP get_Script(IDispatch **ppScript)
	{
		static AutoHotkeyScript sInstance;
		*ppScript = &sInstance;
		return S_OK;
	}
	...
	{
		return VarCollection::Create(&g_script->mDefaultModule.mVars, ppVars);
	}
	...
	STDMETHODIMP get_Ready(VARIANT_BOOL *pValue)
	{
		*pValue = g_script && g_script->mIsReadyToExecute ? VARIANT_TRUE : VARIANT_FALSE;
		return S_OK;
	}


	STDMETHODIMP get_ThreadID(DWORD *pValue)
	{
		*pValue = g_MainThreadID;
		return S_OK;
	}
};
...
bool LibNotifyProblem(LPCTSTR aMessage, LPCTSTR aExtra, Line *aLine, bool aWarn)
{
	if (!lib_ProblemCallback)
		return false;
	Line sLine(0, 0, ACT_EXPRESSION, nullptr, 0);
	if (!aLine)
	{
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	...
	case DLL_THREAD_DETACH:
		if (g_script)
			g_script->TerminateApp(EXIT_CRITICAL, 0);
		if (lib_ProblemCallback && !lpvReserved)
			lib_ProblemCallback->Release(), lib_ProblemCallback = nullptr;
...
{
	return CreateDispatchInstance<AutoHotkeyLib>(ppInst);
}